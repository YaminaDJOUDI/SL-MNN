...
from custom_models.proposed_model import ModelGenerator, HL_MNNGenerator
...
hlmnn = HL_MNNGenerator(input_layer, 10)
...
hlmnn.add_sub_model(layers=snn_layers, name="model_OvR",
...
hlmnn.add_sub_model(layers=snn_layers, name="model_OvO",
                    loss=tf.keras.losses.BinaryCrossentropy(from_logits=True),
                    labels=sub_model_labels)
...
hlmnn.add_sub_model(layers=snn_layers, name="model_SuperClass",
                    loss=tf.keras.losses.BinaryCrossentropy(from_logits=True),
                    labels=sub_model_labels)

optimizer = tf.keras.optimizers.SGD(momentum=0.9, nesterov=True)
final_layers = [Dense(512, activation='sigmoid'), Dense(10, activation='softmax', name='final_output')]
hlmnn.finalize_model(final_layers, name="experiment1_model")

hlmnn.compile_sub_models(optimizer=optimizer, metrics=["accuracy"])
hlmnn.fit_sub_models(x_train, y_train, epochs=150,
                     batch_size=320, validation_data=(x_val, y_val))
hlmnn.final_model.compile(optimizer=optimizer,
                          loss=tf.keras.losses.SparseCategoricalCrossentropy(from_logits=True),
                          metrics=["accuracy"])
hlmnn.final_model.fit(x_train, y_train, epochs=100, batch_size=320, validation_data=(x_val, y_val))
